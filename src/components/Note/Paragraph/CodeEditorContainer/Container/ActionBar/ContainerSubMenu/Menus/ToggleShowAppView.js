...
export default function ToggleShowAppViewButton(props) {
  ...
  const isHide = useMemo(
    () =>
      ...
      Container.meta.config.hide.appView,
    [Container.meta.config.hide]
  );
  const onToggleAppView = useCallback(
    e => {
      e.preventDefault();
      const showConfig = Container.meta.config.hide || { appView: false };
      onUpdateMetaConfig({
        ...Container.meta.config,
        hide: { ...showConfig, appView: !showConfig.appView }
      });
    },
    [isHide]
  );
  ...
  return (
    <MenuItem
      ...
      text={isHide ? `Hidden App view` : `Visible App view`}
